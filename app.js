...
app.post("/login", async (request, response) => {
    ...
    if (dbUser === undefined) { 
        ...
        const isPassword Matched = await bcrypt.compare (password, dbUser.password);
...
app.get("/tweets/:tweetId", authenticateToken, async (request, response) => {
    ...
    const getTweetDetailsQuery = ` 
        ...
        FROM
            tweet INNER JOIN Like ON tweet tweet id = like tweet id INNER JOIN reply ON reply.tweet id = tweet.tweet_id
        WHERE
...
app.get (
    ...
    const getRepliedUsersQuery = `
        SELECT
            *
        FROM
            follower INNER JOIN tweet ON tweet.user_id = follower.following_user_id INNER JOIN reply ON replay.tweet_id = tweet.tweet_id
            INNER JOI N user ON user.user_id = reply.user_id 
        WHERE
        tweet``.tweet_id = ${tweetId} AND follower.follower_user_id = ${user_id}
    ;`;
...
app.get("/user/tweets", authenticateToken, async (request, response) => {
    ...
    const getTweetsDetailsQuery = `
        SELECT
            ...
            tweet.date_time AS dateTime
        FROM
            user INNER JOIN tweet ON user.user_id = tweet.user_id INNER JOIN like ON like.tweet_id = tweet_id = tweet.tweet_id INNER JOIN reply ON replay.tweet_id = tweet.tweet_id
        WHERE
...
app.delete("/tweets/:tweetId", authenticateToken, async (request, response) => {
    ...
    const { payload= request;
    const { user_id, name, username, gender } = payload;
    const selectUserQuery = `SELECT * FROM tweet WHERE tweet.user_id = ${user_id} AND tweet.tweet_id  =  ${tweet};`;
